...
const NewsCard = ({ title, excerpt, image, category, time, isLarge = false, categoryColor = "bg-accent" }: NewsCardProps) => {
  return (
    <article className={`news-card group cursor-pointer ${isLarge ? "md:col-span-2" : ""}`}>
      <div className={`relative overflow-hidden ${isLarge ? "h-64 md:h-80" : "h-48"}`}>
        ...
        <Badge className={`absolute top-3 left-3 ${categoryColor} text-white text-xs`}>{category}</Badge>
      </div>
      <div className="p-4">
        ...
        <p className="text-muted-foreground text-sm line-clamp-2 mb-3 font-body">{excerpt}</p>
