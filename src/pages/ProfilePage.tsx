      ...
      <div className="space-y-6 max-w-2xl">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2"><Label>ईमेल</Label><Input value={user?.email ?? ""} disabled /></div>
            <div className="space-y-2"><Label>पूरा नाम</Label><Input value={fullName} onChange={(e) => setFullName(e.target.value)} /></div>
            <Button onClick={handleSave} disabled={saving}>{saving ? "सहेज रहे हैं..." : "अपडेट करें"}</Button>
          </CardContent>
        </Card>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[{ label: "कुल लेख", value: stats.total, icon: FileText }, { label: "स्वीकृत", value: stats.approved, icon: CheckCircle }, { label: "लंबित", value: stats.pending, icon: Clock }, { label: "कुल व्यू", value: stats.views, icon: Eye }].map((s) => (
            <Card key={s.label}><CardContent className="pt-4 flex flex-col items-center gap-1"><s.icon className="w-5 h-5 text-primary" /><span className="text-2xl font-bold">{s.value}</span><span className="text-xs text-muted-foreground">{s.label}</span></CardContent></Card>
          ))}
        </div>
      </div>
    </DashboardLayout>
