...
const AdminStats = () => {
  ...
  useEffect(() => { supabase.from("articles").select("title, views, status").order("views", { ascending: false }).limit(10).then(({ data }) => setTopArticles(data ?? [])); }, []);
    ...
    <DashboardLayout type="admin">
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            {topArticles.length === 0 ? <p className="text-muted-foreground">कोई डेटा नहीं</p> : (
              <ol className="space-y-2">
                {topArticles.map((a, i) => (
                  <li key={i} className="flex justify-between items-center py-2 border-b border-border last:border-0">
                    <span className="text-sm"><span className="font-bold mr-2">{i + 1}.</span>{a.title}</span>
