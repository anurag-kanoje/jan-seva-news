...
interface UserWithRole { user_id: string; full_name: string; created_at: string; role: string | null; role_id: string | null; }
...
const AdminUsers = () => {
  ...
  const assignRole = async (userId: string, role: string) => {
    ...
    toast({ title: `भूमिका "${role}" सेट की गई` }); fetchUsers();
  };
  const removeRole = async (userId: string) => {
    const { error } = await supabase.from("user_roles").delete().eq("user_id", userId);
    if (error) { toast({ title: "त्रुटि", description: error.message, variant: "destructive" }); return; }
    toast({ title: "भूमिका हटाई गई" }); fetchUsers();
  };
    ...
    <DashboardLayout type="admin">
      <div className="space-y-4">
        ...
        <div className="bg-card rounded-lg border border-border">
          <Table>
            <TableHeader><TableRow><TableHead>नाम</TableHead><TableHead>भूमिका</TableHead><TableHead>जुड़ने की तिथि</TableHead><TableHead>कार्रवाई</TableHead></TableRow></TableHeader>
            <TableBody>
              {loading ? <TableRow><TableCell colSpan={4} className="text-center py-8">लोड हो रहा है...</TableCell></TableRow>
              : users.length === 0 ? <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-8">कोई उपयोगकर्ता नहीं</TableCell></TableRow>
              : users.map((u) => (
                <TableRow key={u.user_id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Select value={u.role ?? ""} onValueChange={(val) => assignRole(u.user_id, val)}><SelectTrigger className="w-32 h-8"><SelectValue placeholder="भूमिका चुनें" /></SelectTrigger><SelectContent><SelectItem value="writer">लेखक</SelectItem><SelectItem value="admin">एडमिन</SelectItem></SelectContent></Select>
                      {u.role && <Button variant="ghost" size="icon" onClick={() => removeRole(u.user_id)} title="भूमिका हटाएं"><Trash2 className="w-4 h-4 text-destructive" /></Button>}
                    </div>
                  </TableCell>
                </TableRow>
              ))}
