...
const ProductionChecklist = () => {
  ...
  const backfillSlugs = async () => {
    setBackfilling(true); setResult(null);
    try {
      const { data: articles, error } = await supabase.from("articles").select("id, title").is("slug", null);
      if (error) throw error;
      if (!articles || articles.length === 0) { setResult("सभी लेखों में पहले से slug है। कोई बदलाव नहीं।"); setBackfilling(false); return; }
      ...
      for (const article of articles) {
        ...
        while (existingSlugs.has(slug)) slug = generateUniqueSlug(article.title);
        existingSlugs.add(slug);
        const { error: updateErr } = await supabase.from("articles").update({ slug }).eq("id", article.id);
    ...
    } catch (err: any) { setResult(`त्रुटि: ${err.message}`); }
    ...
    <DashboardLayout type="admin">
      <div className="max-w-2xl space-y-6">
        ...
        <div className="p-4 bg-card rounded-lg border border-border space-y-3">
          ...
          <p className="text-sm text-muted-foreground">जिन लेखों में slug नहीं है, उनमें title से slug बनाएं।</p>
          <Button onClick={backfillSlugs} disabled={backfilling} variant="outline">{backfilling && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}{backfilling ? "Processing…" : "Backfill Slugs"}</Button>
        ...
        <div className="space-y-3">
          {checks.map((item, i) => (
            <div key={i} className="flex items-center gap-3 p-3 bg-card rounded-lg border border-border">
              {item.done ? <CheckCircle className="w-5 h-5 text-green-600 shrink-0" /> : <XCircle className="w-5 h-5 text-destructive shrink-0" />}
